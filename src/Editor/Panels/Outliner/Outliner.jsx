...
class Outliner extends Component {
    ...
    if (d1 > d2) {
      for (let i = 0; i < d_diff; i++) {
        ...
        if (ob1.classname === "Frame") {
          indices1.unshift(
            ob1.getChildren().length - 1 - ob1.getChildren().indexOf(temp)
          );
        } else {
      ...
      for (let i = 0; i < d_diff; i++) {
        ...
        if (ob2.classname === "Frame") {
          indices2.unshift(
            ob2.getChildren().length - 1 - ob2.getChildren().indexOf(temp)
          );
        } else {
  ...
  isActive = (object) => {
    ...
    if (object.classname === "Layer") {
      ...
      return (
        object.start <= this.props.project.activeTimeline.playheadPosition &&
  ...
  select = (e, indices) => {
    ...
    let object = this.getObjectAtIndices(
    ...
    if (e.shiftKey && is_same_depth) {
      let { ancestor, indices1, indices2 } = this.getCommonAncestorIndices(
  ...
  render() {
    ...
    return (
      <div
        className={classNames("docked-pane outliner", this.props.className)}
        aria-label="Outliner"
      >
                ...
                <OutlinerObject
                  ...
                  toggle={(e, indices, property) => {
                    ...
                    if (property === "select") {
                      ...
                      let layer = this.getObjectAtIndices(
                        this.props.project.activeTimeline,
                        indices,
                        indices.length
                      );
