    ...
    drop: (item, monitor) => {
      ...
      if (item.type === type) {
        ...
        if ((hoverClientY <= hoverMiddle) ^ (type === "object")) {
          ...
          moveSelection(
            data.parent,
            data.parent.getChildren().indexOf(data) + 1
          );
        }
      } else if (type === "layer") {
        if (item.type === "object") {
          if (data.activeFrame) {
            moveSelection(
              data.activeFrame,
              data.activeFrame.getChildren().length
            );
          }
        }
      } else if (type === "frame") {
    ...
    <>
      ...
      >
        ...
        >
          ...
          {object_name && <span className="outliner-name">{object_name}</span>}
