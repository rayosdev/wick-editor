...
const SPECTRUM_SWATCHES: string[] = [
  "#D0021B",
  "#F8E71C",
  "#7ED321",
  "#4A90E2",
...
const WickColorPicker: React.FC<WickColorPickerProps> = (props) => {
  ...
  const currentColor = useMemo(() => hsvaToRgbaString(hsva), [hsva]);
  const hueBaseColor = useMemo(
    () => tinycolor({ h: hsva.h, s: 1, v: 1 }).toHexString(),
    [hsva.h],
  );

  const emitHsva = (nextHsva: HSVAColor): void => {
  ...
  const renderSwatchColumn = (colorList: string[], index: number): JSX.Element => {
    return (
      <div
        key={`swatch-color-column-${index}`}
        ...
        {colorList.map((color, rowIndex) => {
          return (
            <WickSwatch
              ...
              key={`swatch-color-${color}-${rowIndex}`}
            />
          );
        })}
      </div>
    ...
    return (
      <div className="wick-swatch-picker-book -mx-[10px] my-[5px] flex flex-row flex-wrap pb-0 pl-[10px] pr-0 pt-[5px]">
        {colors.map((colorList, index) => {
          return renderSwatchColumn(colorList, index);
        })}
      </div>
    );
  };
    ...
    return (
      <div className="wick-color-picker h-[300px] w-[220px] rounded-[4px] bg-editor-primary px-[10px] pb-0 pt-[10px]">
        ...
        <div className="wick-swatch-color-picker-body flex flex-col">
          {renderSwatchbook(SWATCH_COLORS)}
        </div>
      </div>
    );
  };

  const renderSpectrumSwatches = (colors: string[]): JSX.Element => {
    return (
      <div className="wick-color-picker-swatches-container -mx-[10px] my-[5px] flex h-[25px] flex-wrap border-t border-solid border-black/10 pb-0 pl-[10px] pr-0 pt-[5px]">
        {colors.map((color, index) => {
          const isSelected = tinycolor(color).toHexString() === currentHex;
          return (
            <button
              key={`color-swatch-${color}-${index}`}
              type="button"
              className="wick-color-picker-small-swatch mb-[10px] mr-[10px] h-[16px] min-w-[16px] w-[16px] overflow-hidden rounded-[4px] border-[1px] border-solid border-[#222] p-0"
              style={{
                ...CHECKERBOARD_STYLE,
              ...
              onClick={() => props.onChangeComplete(parseHexToPickerColor(color))}
            />
          );
        })}
      </div>
  ...
  const renderSpectrum = (): JSX.Element => {
    ...
    return (
      <div className="wick-color-picker h-[300px] w-[220px] rounded-[4px] bg-editor-primary px-[10px] pb-0 pt-[10px]">
        {renderHeader()}
        <div
          className="wick-color-picker-saturation relative mt-[5px] h-[115px] w-full cursor-crosshair overflow-hidden rounded-[2px]"
          style={{ backgroundColor: hueBaseColor }}
          onPointerDown={(event) => {
            const rect = event.currentTarget.getBoundingClientRect();
            const ratioX = clamp((event.clientX - rect.left) / rect.width, 0, 1);
            const ratioY = clamp((event.clientY - rect.top) / rect.height, 0, 1);
            emitHsva({ ...hsva, s: ratioX, v: 1 - ratioY });
        ...
        <div className="wick-color-picker-control-body mt-[5px] flex w-full flex-row">
          ...
          <div id="wick-color-picker-bar-container" className="w-[140px]">
            <div className="wick-color-picker-control-bar relative mb-[2.5%] h-[11px] w-[140px] overflow-hidden bg-white">
            ...
            {!props.disableAlpha && (
              <div
                className="wick-color-picker-control-bar relative mb-[2.5%] h-[11px] w-[140px] overflow-hidden bg-white"
        ...
        <div className="mt-[6px] grid grid-cols-5 gap-[4px]">
          ...
          <input
            ...
            onChange={(event) =>
              emitHsva(toHsva(`rgba(${clamp(Number(event.target.value), 0, 255)},${currentRgb.g},${currentRgb.b},${currentRgb.a})`))
            }
          />
          <input
            ...
            className="h-[24px] rounded-[3px] border border-black/30 bg-[#4f4f4f] px-[4px] text-[11px] text-white"
            onChange={(event) =>
              emitHsva(toHsva(`rgba(${currentRgb.r},${clamp(Number(event.target.value), 0, 255)},${currentRgb.b},${currentRgb.a})`))
            }
          />
          <input
            ...
            className="h-[24px] rounded-[3px] border border-black/30 bg-[#4f4f4f] px-[4px] text-[11px] text-white"
            onChange={(event) =>
              emitHsva(toHsva(`rgba(${currentRgb.r},${currentRgb.g},${clamp(Number(event.target.value), 0, 255)},${currentRgb.a})`))
            }
          />
        </div>
        {renderSpectrumSwatches(SPECTRUM_SWATCHES)}
        {renderSpectrumSwatches(lastColors)}
      </div>
    );
  };
