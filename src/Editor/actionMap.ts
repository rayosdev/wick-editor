 ...
 */
export interface ActionMapEditor {
  ...
  createButtonFromSelection: () => void;
...
export type EditorActionsMap = Record<string, EditorAction>;

class ActionMapInterface {
  public editorActions: EditorActionsMap = {};

  constructor(private editor: ActionMapEditor) {
    this.createEditorActions();
  }

  /**
   * Creates action objects for use around the editor.
   * Action objects must include properties {function}action and a {string}id representing a unique id.
   * Optional properties include a {string}color, {string}icon, and {string}tooltip.
   */
  private createEditorActions(): void {
    this.editorActions = {
      flipHorizontal: {
        icon: 'flipHorizontal',
