...
class ScriptInfoInterface extends Object {
    ...
    get scriptsByType(): ScriptsByType {
        return {
     ...
     * @param {ScriptData} scriptA - script object
     * @param {ScriptData} scriptB - script object
     * @returns {number} negative if script A comes before b, positive if b comes before a, 0 if they are ordered the same.
     */
    sortScripts = (scriptA: ScriptData, scriptB: ScriptData): number => {
        const typeA = this.getScriptType(scriptA.name);
        const typeB = this.getScriptType(scriptB.name);
     ...
     */
    getScriptType = (name: string): string | null => {
    ...
    get scriptTypeColors(): ScriptTypeColors {
        return {
            'Timeline': 'blue',
            'Mouse': 'green',
            'Keyboard': 'yellow',
    ...
    get scriptDescriptions(): ScriptDescriptions {
        return {
    ...
    get referenceItems(): ReferenceItems {
        return {
    ...
    get timelineReference(): ReferenceItem[] {
        return [
            {
                name: 'play',
                snippet: 'play()',
    ...
    get objectReference(): ReferenceItem[] {
        return [
            {
                name: 'x',
                snippet: 'this.x',
    ...
    get soundReference(): ReferenceItem[] {
        return [
            {
                name: 'playSound',
    ...
    get projectReference(): ReferenceItem[] {
        return [
            {
                name: 'project.width',
    ...
    get randomReference(): ReferenceItem[] {
        return [
            {
                name: 'random.integer',
    ...
    get inputReference(): ReferenceItem[] {
        return [
            {
                name: 'mouseX',
                snippet: 'mouseX',
    ...
    get eventReference(): ReferenceItem[] {
        const events: ReferenceItem[] = [];
        const descriptions = this.scriptDescriptions;
