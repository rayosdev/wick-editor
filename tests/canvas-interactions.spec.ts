...
test.describe('Canvas Mouse/Trackpad Interactions', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.locator('#canvas-container-wrapper').waitFor({ state: 'visible', timeout: 10000 });
  });
...
test.describe('Cross-browser Compatibility', () => {
  ...
  test('canvas interactions work in WebKit/Safari', async ({ page, browserName }) => {
  ...
  test('should throttle with requestAnimationFrame', async ({ page }) => {
    ...
    const updateCount = await page.evaluate(async () => {
      ...
      const originalZoomSetter = Object.getOwnPropertyDescriptor(
        Object.getPrototypeOf(view), 'zoom'
      )?.set;
      ...
      const canvas = document.querySelector('#canvas-container-wrapper canvas') as HTMLCanvasElement;
      for (let i = 0; i < 20; i++) {
